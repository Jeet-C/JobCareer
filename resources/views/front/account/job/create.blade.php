        ...
        <div class="row">
            ...
            <div class="col-lg-9">
               ...
               {{-- @include('front.layout.alert') --}}
                    <form action="" method="post" id="saveJobeForm" name="saveJobeForm">
                        <div class="card border-0 shadow mb-4">
                                        ...
                                        <select name="category" id="category" class="form-control">
                                            ...
                                            @if ($category->isNotEmpty())
                                                @foreach ($category as $jobtype )
                                                    <option value="{{ $jobtype->id }}">{{ $jobtype->category_name }}</option> 
                                                @endforeach     
                                            @endif
                            ...
                            <div class="card-footer  p-4">
                                <button type="sub" class="btn btn-primary">Save Job</button>
                            </div>  
                        </div>        
                    </form>          
            </div>
...
$(document).ready(function(){
    $('#saveJobeForm').submit(function(e){
        e.preventDefault();
        $("button[type='submit']").prop('disabled',true);
        $.ajax({
            url : '{{ route("account.jobPost") }}',
            type : 'post',
            dataType : 'json',
            data :   $('#saveJobeForm').serializeArray(),
